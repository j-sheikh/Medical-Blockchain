...
class Node(threading.Thread):
    ...
    def connect_with_node(self, host, port, reconnect=False):
        ...
        # print(f'inside{host}:{port}')
        if host == self.host and port == self.port:
            print("connect_with_node: Cannot connect with yourself!!")
            return False
        
        # print('Checking if node is already connected with this node!')
        ...
        try:
            ...
            try:
                connected_node_id = sock.recv(8192).decode('utf-8') # When a node is connected, it sends its id!
            except socket.timeout:
                # handle the timeout exception
                print("Timeout reached, no data received.")
                return False
            # print(f'Received node id: {connected_node_id}')
            # Cannot connect with yourself
            if self.id == connected_node_id:
                print("connect_with_node: You cannot connect with yourself?! Closing socket...")
            ...
            # print('Checking inbound connections...')
            for node in self.nodes_inbound:
                if node.host == host and node.id == connected_node_id:
                    print("connect_with_node: This node (" + node.id + ") is already connected with us. Closing socket...")
                    sock.send("CLOSING: Already having a connection together".encode('utf-8'))
                    sock.close()
                    return True
    ...
    def disconnect_with_node(self, node):
        ...
        if node in self.nodes_outbound:
            ...
            # node.socket.close()
            node.node_disconnect_with_outbound_node(self)
            node.nodes_inbound.remove(self)
        else:
